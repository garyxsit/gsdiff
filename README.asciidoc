...
Therefore, neither of these cases can conclude it is a simile because looking into `as` or `as &#x002E;&#x002E;&#x002E; as` is only one of the sufficient conditions. That is also one of the reasons that metaphors can only be concluded by concerning its purpose at the same time, because not all metaphors are in the form of `(Subject) is (Object)` where the subject can be a noun, the object can be a noun or an adjective, and `is` can be varied from its tenses and corresponding meaning as this pattern is barely one of its forms, just as a multi-table join SQL can actually be written in other representations that do not mention `JOIN` keywords. +
